...
# Full
lc_made_model <- glmmTMB(LCM ~ YearSinceFlanged + FAI + NrFemAssoc + total.rain + LCH +offset(ObsTime) +(1|Name/Month/Year),
                         data=data, 
                         family=poisson(link = "log"))
...
lc_nr_pulses_full <- glmmTMB(AvgPulseNr ~ YearSinceFlanged + FAI + NrFemAssoc + total.rain + LCH +offset(AvgDuration) +(1|Name/Year/Month),
                              data=data, 
                              family=poisson(link = "log"))
...
lc_duration_full_0 <- glmmTMB(AvgDuration ~ YearSinceFlanged + FAI + NrFemAssoc + LCH + total.rain + (1|Name/Month/Year),
...
lc_duration_full <- glmmTMB(AvgDuration ~ YearSinceFlanged + FAI + NrFemAssoc + LCH + total.rain + (1|Name),
                            data=data, family = gaussian())
